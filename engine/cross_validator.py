# ============================================================================
# 交叉验证 B86-B95 Bug 审计标记
# ============================================================================
# B86 修复: 8术权重 - 不同术法对不同问题的权重
# B87 修复: 冲突处理 - 八术结果不一致时如何处理
# B88 修复: 维度覆盖 - 8术是否覆盖所有维度
# B89 修复: 置信度 - 每个术法结果的可信度评分
# B90 修复: 108人物 - 视角引擎人物数据库
# B91 修复: LLM 调用 - 错误重试/超时
# B92 修复: 辩论流程 - 多视角辩论的流程控制
# B93 修复: 溟玄风格 - 极简大实话的语言规范
# B94 修复: 禁用词 - 22 个 AI 检测词
# B95 修复: QA 引擎 - 自动问题回答
# ============================================================================
...
from .udm import DestinyModel, GAN_WUXING as GAN_WUXING_TUPLES
...
class CrossValidator:
    ...
    def _validate_personality(self) -> List[ConsensusItem]:
        ...
        if self.udm.features:
            ...
            for f in self.udm.features:
                ...
                elif "财" in f and "不显" not in f and "务实" not in matched_traits:
    ...
    def _validate_health(self) -> List[ConsensusItem]:
        ...
        if counts and any(v > 0 for v in counts.values()):
            ...
            _GAN_WX = {k: v[0].value for k, v in GAN_WUXING_TUPLES.items()}
            ...
            if hidden_counts[max_wx] >= 4:
                ...
                items.append(ConsensusItem(
                    aspect="健康体质",
                    finding=f"{max_wx}过旺（{hidden_counts[max_wx]}个，含藏干中气余气），注意{organs}功能",
                    supporting_methods=["八字"],
                    confidence=ConfidenceLevel.MEDIUM
                ))
            if counts[min_wx] == 0:
    ...
    def _detect_conflicts(self) -> List[ConflictItem]:
        ...
        if any("比劫" in f for f in features) and any("财星" in f and "生" not in f and "美" not in f and "不显" not in f for f in features):
            bazi_wealth_good = False
        ...
        if self.udm.liuyao_chart:
            ...
            bazi_wealth_loss = any("比劫" in f for f in features) and any("财" in f and "生" not in f and "美" not in f and "不显" not in f for f in features)
