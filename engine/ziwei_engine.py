...
- 公式 13: P(ω_t | H_{t-1}) = P(ω_t) — 紫微排盘结果对未来事件无预测优势
- 本引擎的"四化飞星"、"自化"、"大限流年"是对**命盘结构**的静态描述,不构成预测
- 紫微斗数有"主观解释"的强依赖(同一命盘不同派别解释可能冲突),任何"客观预测"输出应被理解为解读层选择
- 不存在"紫微推下期彩票号"的数学通路 — 本引擎输出仅用于命盘解释
"""
