...
数学边界（基于大乐透概率论 14 公式,2026-07-09 梧指令补充）:
- 公式 13: P(ω_t | H_{t-1}) = P(ω_t) — 历史条件概率等于无条件概率
- 本引擎的"五行补缺"、"调候"、"旺衰"都是对**当前时刻**的描述,不是对未来号码的预测
- 不存在"基于八字推下期彩票号"的数学通路 — 本引擎输出仅用于命盘解释
- 任何"用八字做彩票"的输出应被理解为随机数(参考 api/lottery_routes.py 的均匀抽样)
- 真太阳时修正的误差上界:经度 ±0.5°(约 ±2 分钟),均时差 ±30 秒
- 真太阳时修正**不**改变命盘定性结论(身强/身弱/喜忌五行),只影响精确分钟级排盘
"""
...
# ─── 德秀贵人（以月支查天干组合）────────────────
SHENSHA_DEXIU_MAP = {
    # 德=月德天干（三合局本气），秀=生德之五行天干
    # 《三命通会》: 寅午戌月丙为德甲乙为秀, 申子辰月壬为德庚辛为秀,
    #              亥卯未月甲为德壬癸为秀, 巳酉丑月庚为德丙丁为秀
    '子': {'德':'壬', '秀':'庚辛'},   # 申子辰(水)月德壬，金生水为秀
    '丑': {'德':'庚', '秀':'丙丁'},   # 巳酉丑(金)月德庚，火克金为秀
    '寅': {'德':'丙', '秀':'甲乙'},   # 寅午戌(火)月德丙，木生火为秀
    '卯': {'德':'甲', '秀':'壬癸'},   # 亥卯未(木)月德甲，水生木为秀
    '辰': {'德':'壬', '秀':'庚辛'},   # 申子辰(水)月德壬，金生水为秀
    '巳': {'德':'庚', '秀':'丙丁'},   # 巳酉丑(金)月德庚，火克金为秀
    '午': {'德':'丙', '秀':'甲乙'},   # 寅午戌(火)月德丙，木生火为秀
    '未': {'德':'甲', '秀':'壬癸'},   # 亥卯未(木)月德甲，水生木为秀
    '申': {'德':'壬', '秀':'庚辛'},   # 申子辰(水)月德壬，金生水为秀
    '酉': {'德':'庚', '秀':'丙丁'},   # 巳酉丑(金)月德庚，火克金为秀
    '戌': {'德':'丙', '秀':'甲乙'},   # 寅午戌(火)月德丙，木生火为秀
    '亥': {'德':'甲', '秀':'壬癸'},   # 亥卯未(木)月德甲，水生木为秀
}
...
GAN_LIUHE = dict(GAN_HE)  # 天干五合（天德合、月德合复用此常量，独立副本防误改）
...
class BaziEngine(DivinationEngine):
    ...
    def analyze(self, time: CorrectedTime, gender: int) -> dict:
        ...
        try:
            # B08 修复说明 (2026-07-23 复核, 梧真盘 8 case 全过):
            # lunar_python 库本身已正确实现 getYun(gender) 的顺/逆排。
            # 原代码此处有一段 "direction 标记 + 手动反转" 的死代码 (line 682-694),
            # 实际从未执行任何 reverse, 仅留注释 "如果方向不对, 标记 warning (不实际反转, 因为无法修改) pass"
            # 这段代码会误导后来人以为是 "未修 bug", 实际行为已对。
            # 现删除: 由 tests/test_dayun_direction.py (8 case) 防回归。
    ...
    def _extract_features(self, year, month, day, time_pillar, shishen_gan, hidden_gans, day_master: str = '') -> list:
        ...
        # 伤官配印：伤官与正印同透——伤官的叛逆创造力被正印的学识涵养所驾驭
        # 《子平真诠》列为上格：伤官虽凶，有印则吉。才华有了底蕴，表达有了深度。
        # 注：偏印(枭神)+伤官性质不同，偏印夺食之性会干扰伤官的表达，不归入伤官配印
        if has_shangguan and has_zhengyin:
