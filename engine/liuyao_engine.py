...
# B42 修复: 动爻 - 根据纳甲起卦
# B43 修复: 变卦 - 动爻阴阳反转
# B44 修复: 世应 - 世爻(主问者), 应爻(对方)
# B45 修复: 六亲 - 父母、兄弟、子孙、妻财、官鬼
# B46 修复: 六神 - 青龙、朱雀、勾陈、螣蛇、白虎、玄武
# B47 修复: 用神 - 根据问事选取的爻
#
# 数学边界（2026-07-09 梧指令补充）:
# - 公式 13: P(ω_t | H_{t-1}) = P(ω_t) — 六爻起卦结果对未来事件无预测优势
# - 六爻样本空间 |Ω| = 64卦 × 6爻 = 4096 种组合(忽略变卦则更小)
# - 但"用神选取"、"世应分析"、"动爻判断"含有解释层主观性,任何"客观预测"输出应被理解为解读层选择
# - 不存在"六爻推下期彩票号"的数学通路 — 本引擎输出仅用于占问解释
# B48 修复: 旺衰 - 月建日辰对爻的影响
# B49 修复: 月破日破 - 月破=全破, 日破=暗动
# B50 修复: 伏神 - 本卦缺六亲时的伏藏
# ============================================================================
...
class LiuYaoEngine(DivinationEngine):
    ...
    def _analyze_najia(self, time: CorrectedTime, gender: int) -> dict:
        ...
        # 过滤dong列表中的无效值（非int类型、None、负数、越界索引）
        dong = list(dict.fromkeys(d for d in dong if isinstance(d, int) and 0 <= d < 6))  # 过滤+去重，保序
