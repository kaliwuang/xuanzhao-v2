...
local_guji: C:\Users\W\AppData\Local\Temp\qimen_guji\遁甲符应经-宋-杨维德.txt
...
```python
def 遁甲符应经_pan(dt, ju_kind="auto"):
    """依据《遁甲符应经》起例排盘。"""
    ...

def 遁甲符应经_ju_check(pan):
    """按《遁甲符应经》格清单检测当前盘。"""
    ...

def 遁甲符应经_chao_jie(qi_jie, fu_tou):
    """超神/接气/正授判定。"""
    ...
```
...
- **底本**:https://raw.githubusercontent.com/kentang2017/shushubook/main/奇门遁甲/遁甲符应经-宋-杨维德.txt
- **转码**:UTF-16 LE → UTF-8,Python 一次性转,无字符损失。
- **起止行号**:电子本第 1 行至第 545 行,共 12433 字符。
- **异体字**:保留原档用字,不擅改。
- **完整度自评**:100%。原文 545 行已逐字 quote 完整保留;字句疏解与实战规则待人工逐条补全(参考 遁甲符应经 电子本)。
