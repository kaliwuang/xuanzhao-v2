#!/usr/bin/env python3
"""Add radar chart rendering to index.html"""
import os

filepath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'frontend', 'index.html')

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

old = "        if (data.xingming) { document.getElementById('xingmingPanel').classList.remove('hidden'); renderXingming(data.xingming); }\n    }\n\n    /* ===================== 八字 ===================== */"

new_call = "        if (data.xingming) { document.getElementById('xingmingPanel').classList.remove('hidden'); renderXingming(data.xingming); }\n        renderRadarChart(data);\n    }\n\n"
...
radar_func = r"""    /* ===================== 雷达图 ===================== */
    function renderRadarChart(data) {
        ...
        var love = 55;
        if (bazi.day) {
            var dayZhi = bazi.day[1];
            if ('子午卯酉'.indexOf(dayZhi) >= 0) love += 10;
        }
        ...
        ctx.closePath();
        ctx.fillStyle = 'rgba(201, 169, 110, 0.15)'; ctx.fill();
        ...
        keys.forEach(function(k, i) {
            ...
            html += '<div class="radar-score-item">'
                + '<span class="radar-score-label">' + k + '</span>'
                + '<div class="radar-score-bar"><div class="radar-score-fill" style="width:' + v + '%;background:' + color + '"></div></div>'
                + '<span class="radar-score-value" style="color:' + color + '">' + v + '</span>'
                + '</div>';
        });
...
if old in content:
    content = content.replace(old, new_call + radar_func + "    /* ===================== 八字 ===================== */")
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print('OK: radar chart added to index.html')
else:
    print('ERROR: pattern not found in index.html')
    # Show nearby content
    idx = content.find('renderXingming')
    if idx >= 0:
        print('Near renderXingming:', repr(content[idx:idx+200]))
